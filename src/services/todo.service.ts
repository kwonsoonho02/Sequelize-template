...
export class TodoService {
  ...
  public async createTodo(
    ...
    const createTodoData: Todo = await DB.TodoLists.create({
      ...todoData,
      userId,
    });
  ...
  public async updateTodo(
    ...
    const [affectCount] = await DB.TodoLists.update(todoData, {
      where: { userId, id: todoId },
    });

    if (affectCount === 0) return false;
    return true;
  }

  public async deleteTodo(userId: number, todoId: number): Promise<number> {
    const deleteTodoData: number = await DB.TodoLists.destroy({
      where: { userId, id: todoId },
    });
