...
export class TodoListController {
    ...
    public getTodo = async (req : RequestWithUser, res : Response, next : NextFunction) => {
        try{
    ...
    public createTodo = async (req : RequestWithUser, res : Response, next : NextFunction) => {
        try{
            ...
            const user = req.user;
            console.log(user)
            const createTodoData : Todo = await this.todo.createTodo(todoData, user.id);
    ...
    public updateTodo = async (req: RequestWithUser, res: Response, next: NextFunction) => {
        try {
            ...
            const updateTodoData : number[] = await this.todo.updateTodo(user.id, todoId,todoData);
    ...
    public deleteTodo = async (req : RequestWithUser, res : Response, next : NextFunction) => {
        try {
            const user = req.user;
            const todoId = Number(req.params.id);
            const deleteTodoData : number = await this.todo.deleteTodo(user.id, todoId);
